...
This is currenty support for the one way communication between two devices with acknowlagement (Reliabble Data Transmission)
Version 3.0
...
# Delay for waiting for a ACK
WAIT = 2

# Maximum number of trys for establish the connection
MAX_TRY = 5
...
# Function to send meadata and waiting for the ACK
def sendMetaData(metaData, radio=sx1278, delay=DELAY):
    ...
    for i in range(MAX_TRY):
        ...
        packet = radio.receive(timeout=2, with_header=True)
...
def sendStrings(rawData, sender_id="DronePi", radio=sx1278, delay=DELAY):
    ...
    try:
        ...
        if not sendMetaData(metadata):
            raise ValueError("Unable to build up a connection.")
